...
/*   Updated: 2020/11/26 10:40:53 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main() {
	...
	try {
		...
		for (std::vector<ConfigServer>::iterator it = config->getServerList().begin(); it != config->getServerList().end(); it++) {
			it->printServer();
		}

		config->getServerUnit(8080, "localhost")->printServer();
